...
abstract class Activerecord
{
    ...
    /**
     * Holds the validation rules for a model
     * 
     * @var array
     */
    ...
    /**
     * Column to group a query by
     * default being the id
     * 
     * @var optional string
     */
    public $group_by = 'id';
    ...
    /**
     * Holds associative data sets for
     * saving associated data to their tables
     * 
     * @var array
     */
     ...
     * @return void
     */
    public function where($condition, $value="", $loose=false)
    {
        if( !!$condition && !!$value ) {
            $condition = DB_SUFFIX.'_'.$condition.'= '.$condition;
        } else {
            if( !$loose ) {
                $condition = DB_SUFFIX.'_'.$condition;
            }
        }
    ...
    public function build_where_clause()
    {
        if( count($this->where_conditions) > 0 ) {
            $result = implode(' '.$this->where_implode.' ', $this->where_conditions);
        } else {
            $result = false;
        }
     ...
     * 
     * @param array $binds
     * @param optional bool $no_group - group a query if set to false
     * 
     * @return assoc array
     */
    public function all($binds=array(), $no_group=true)
    {
        $this->retrieve_has_one_joins();

        if (!$no_group) {
            $options['group'] = 'GROUP BY '.$this->full_table().'.'.$this->group_by;
        }
    ...
    public function insert()
    {
        ...
        $this->attributes['id'] = $output;

        $this->handle_saving_of_associations();

        $this->handle_saving_of_has_one_association();
    ...
    {
        if( property_exists($this, 'has_one') && $this->has_one != "" ) {
    ...
    /**
     * Appends has many to an associative array instance
     * so each iterations has its associations with it
     * 
     * @param array $data
     * 
     * @return bool/array
