...
class Has_many extends Relationships
{
    ...
    /**
     * Builds up the has many associations for an 'all' method
     * looping round and appending each instance to that specific item
     * in an array - as theres no just one value to associate
     * 
     * @param array $data
     * 
     * @return array
    ...
    public function build_all_relationship($data)
    {
        foreach($data as $row) {
            ...
            if( count($has_many) ) {
                $result[] = $this->append_has_many_to_data($has_many, $row);
                
                $this->associations_data = array();
            }
        }
    ...
    /**
     * Append has many instance to an iteration of an array
     *
     ...
     * @return array
     */
    private function append_has_many_to_data($has_many, $data)
    {
